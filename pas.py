            ...
            if flag:                
                ...
                if famousLabel is not None:
                    ...
                    if prevAccuracy > newAcc :
                        ...
                        #print("Prev acc", prevAccuracy)
                        #print("Accuracy increased to ", newAcc)
                        prevAccuracy = newAcc
                        updatedAcc = prevAccuracy
        if topAcc >= updatedAcc:
            break
        else:
            #print("Pruning happening on next level")
            topAcc = updatedAcc
...
def updateTree(nodes):
    for node in nodes:
        ...
        for child in node.children:
            if not node.children[child].isLeaf:
                flag = False
                break
            else:
                if localNodeName is None:
                    localNodeName = node.children[child].label
                elif localNodeName != node.children[child].label:
                    break
                flag = True
        if flag:
            node = localNodeName
